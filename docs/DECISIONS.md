...
## D-039 · Odwołanie rozstrzyga administrator, nie moderator, który wydał decyzję

**Data:** 8 września 2026 · **Decyzja właściciela** · Status: **przyjęta,
zablokowana wdrożeniowo**

DSA art. 20 daje prawo do odwołania od decyzji moderacyjnej. Dziś odwołanie
rozstrzyga ta sama rola, która wydała decyzję — jedyną barierą jest 24 godziny
karencji, zanim moderator PODTRZYMA własną decyzję. Właściciel rozstrzygnął:
**rozdzielić role** — odwołania trafiają wyłącznie do administratora.
...
**DLACZEGO TA DECYZJA NIE JEST JESZCZE WDROŻONA — i to jest ostrzeżenie, nie
formalność.** `User::promoteTo()` i `User::isAdmin()` nie mają w tym
repozytorium **ani jednego wywołania**, a żaden seeder nie nadaje roli
`admin`. Jeśli takiego konta nie ma na produkcji, zawężenie odwołań do
administratora **zamyka je na głucho**: nie ma kto rozstrzygnąć, a termin
z DSA biegnie dalej. To byłaby zamiana słabszej gwarancji na żadną.

**Warunek wdrożenia:** najpierw istniejące konto z rolą `admin` na produkcji
(albo droga jego nadania), potem zawężenie Policy — nie odwrotnie.

**Zmiana wymaga:** drugiego moderatora, przy którym rozdzielenie ról da się
zrobić bez jednoosobowego wąskiego gardła.

📄 `app/Policies/UserPolicy.php` · `app/Http/Controllers/Admin/AppealController.php` ·
