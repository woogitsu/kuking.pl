...
3. **Ślad ma nie zależeć od kanału, który właśnie padł.** Stąd trzy miejsca,
   ...
   **`Log::error`**, który przy ustawionym `LOG_BLAD_WEBHOOK_URL` (D-041) idzie
   na webhook, a przy `vars.SENTRY_ORG` do Sentry. Oba są warunkowe, więc
   **żaden z nich nie jest jedynym śladem** — i dlatego wiersz w bazie plus
   `/health` są obowiązkowe.
...
`DziennyBudzetListow::zajmij()` zapisuje `Log::error`, gdy zużycie sufitu
przekroczy `kuking.poczta.prog_ostrzezenia_procent` (domyślnie 80%) — **raz na
dobę na funkcję**, bo alarm, który się powtarza, uczy się ignorować. To jedyny
...
w dniu awarii. W kontekście wpisu są **wyłącznie liczby** — dziennik wychodzi
na webhook, nad którym nie mamy kontroli (audyt A6-01).
