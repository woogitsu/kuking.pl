...
3. ~~**Zlecić ścieżkę zapisu**~~ — **ZROBIONE 21.09.2026.** Wiersz `w_toku`
   ...
   **Czego ten krok NIE objął:** numer sprawy nie jest jeszcze nigdzie
   ...
   po naszej stronie.
4. **Automat retencji — NAPISANY, ale WYŁĄCZONY. Decyzja właściciela
   z 22.09.2026, `docs/DECISIONS.md` D-233.**

   Wersja z 21.09.2026 włączała kasowanie po 36 miesiącach domyślnie i bez
   przełącznika, uzasadniając to zdaniem „wyłącznik retencji to bezterminowość
   pod inną nazwą". Właściciel rozstrzygnął inaczej: **okresu nie potwierdził
   jeszcze prawnik**, a kasowanie jest **twardym `DELETE`, nieodwracalnym**,
   bez soft-delete i bez eksportu. Pełne uzasadnienie: D-233.

   Co istnieje i działa: `kuking:sprzataj-potwierdzenia-rodo`
   (`App\Domain\Compliance\PrzedawnionePotwierdzeniaRodo`) — liczy od
   `zakonczono` przez `subMonthsNoOverflow` (A6-04), pomija wiersze
   z `wstrzymanie_do` w przyszłości, sprawy w toku trzyma poza kandydatami.

   Co jest wyłączone i **na dwóch barierach naraz**, żeby nie zdejmowała tego
   jedna pomyłka: `kuking.potwierdzenia_rodo.retencja_wlaczona` = `false`,
   a zadanie **nie jest wpięte w `routes/console.php`**. `retention_months`
   jest `null`, nie 36 — samo przestawienie flagi nie uruchomi kasowania według
   niepotwierdzonego progu.

   `--na-sucho` **działa mimo wyłączenia** i niczego nie kasuje; tym mają być
   przygotowane dane historyczne, o których mowa w kroku 5.

   Droga włączenia: §6 niżej. Obie strony pilnuje
   `tests/Feature/RetencjaPotwierdzenRodoTest.php`.
...
7. **Zaktualizować politykę prywatności i `docs/legal/COMPLIANCE.md`** — ocena
   §E wymaga osobnego wiersza dla potwierdzeń z podanym okresem i zakazuje
   chowania wyjątku bezterminowego pod sformułowaniem „według innych zasad".
   **Tego kroku nie da się dziś domknąć uczciwie**, bo okres nie jest ustalony
   (D-233): polityka mogłaby dziś podać wyłącznie „przechowujemy bezterminowo",
   co jest właśnie tym, co ocena §E chce wyeliminować. Krok wraca po §6.
...
## 6. Jak włączyć retencję, gdy prawnik potwierdzi okres

Kasowanie jest dziś wyłączone decyzją właściciela (D-233). **To nie jest
usterka do naprawienia** — nie włączaj tego „przy okazji" ani dlatego, że autor
gałęzi tak chciał. Kolejność niżej jest wiążąca: każdy krok zakłada poprzedni.

1. **Potwierdzić okres u prawnika.** Nie „przyjąć 36 przez analogię" — to była
   analogia do dokumentacji sprawy moderacyjnej (art. 442¹ k.c.), a nie
   ustalenie dla tej kategorii. Zapisać potwierdzony okres i jego uzasadnienie
   w nowej decyzji w `docs/DECISIONS.md`, odsyłającej do D-233.

2. **Ustalić, jak długo żyje kopia zapasowa.** Dopóki polityka prywatności
   mówi „Nie podajemy tu liczby dni, bo nie ustaliliśmy jej jeszcze
   z dostawcą", nie wiadomo, jak długo po `DELETE` istnieje jeszcze droga
   odzysku — czyli nie wiadomo, jak nieodwracalne naprawdę jest kasowanie.
   To jest wiersz P1 z `docs/legal/COMPLIANCE.md` §7 (#193, #594).

3. **Policzyć dane historyczne, zanim cokolwiek zniknie:**

   ```
   php artisan kuking:sprzataj-potwierdzenia-rodo --na-sucho --miesiace=<okres>
   ```

   Działa przy wyłączonej retencji i nie wykonuje żadnego `DELETE`. Pokazuje,
   ile wierszy wpadłoby pod próg i ile jest wstrzymanych udokumentowaną sprawą.
   Jeśli liczba jest niezerowa, **wyeksportować te wiersze przed włączeniem** —
   po `DELETE` nie ma ich skąd odtworzyć inaczej niż z kopii zapasowej.

4. **Ustawić zmienne środowiskowe** (obie, w tej kolejności):

   ```
   KUKING_POTWIERDZENIA_RODO_RETENTION_MONTHS=<potwierdzony okres>
   KUKING_POTWIERDZENIA_RODO_RETENCJA_WLACZONA=true
   ```

   Sama pierwsza nie uruchamia kasowania. Sama druga też nie — bez okresu
   komenda odmawia i mówi dlaczego, zamiast zgadywać.

5. **Wpiąć zadanie w `routes/console.php`** — to druga bariera i bez niej
...
6. **Poprawić `tests/Feature/RetencjaPotwierdzenRodoTest.php`.** Dwa testy są
   ...
   ich zadanie, nie usterka:
   `test_retencja_jest_domyslnie_wylaczona_a_okres_nieustalony`
   i `test_harmonogram_celowo_nie_zna_tej_komendy`. Przepisać je na nowy stan,
   nie kasować: mają dalej pilnować, że okres jest tym potwierdzonym, a nie
   dowolnym.
