...
const temp=mkdtempSync(tmpdir()+'/kuking-empty568-');
...
await page.goto('http://127.0.0.1:8068/szukaj?q=Kalarepa&ile=200&od_przepisu=400');
...
await page.screenshot({path:`output/empty568-${width}-${theme}-top.png`});
await sprawdzTab(page,'/szukaj');
if(await page.getByRole('link',{name:'Wróć do początku przepisów'}).count()!==1)throw Error('Brak powrotu z pustej strony');
await page.waitForTimeout(350);
...
await page.screenshot({path:`output/empty568-${width}-${theme}.png`});
const cdp=await context.newCDPSession(page);
const shot=await cdp.send('Page.captureScreenshot',{format:'png',fromSurface:true,captureBeyondViewport:false});
fs.writeFileSync(`output/empty568-${width}-${theme}-cdp.png`,Buffer.from(shot.data,'base64'));
await cdp.detach();
results.push({theme,zoom,...geometry});
}
await page.getByRole('link',{name:'Wróć do początku przepisów'}).click();
await page.waitForURL(u=>u.searchParams.get('od_przepisu')==='0');
console.log('POWROT_Z_PUSTEGO_OKNA_OK');
writeFileSync('output/empty568.json',JSON.stringify(results,null,2));console.log(results);
