...
for name,file,old,new,cmd in cases:
 ...
 try:
  ...
  if cmd==browser:subprocess.run(['npm','run','build'],cwd=r,env=env,stdout=subprocess.DEVNULL,check=True)
...
(out/'negatywy.json').write_text(json.dumps(results,indent=2));print('4 negatywy i 4 przywrócenia PASS')
