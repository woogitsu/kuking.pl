...
| railway-iac / `apply` | Wyłącznie ręcznie (`workflow_dispatch` z `main`, wpisane potwierdzenie „STOSUJE PLAN PRODUKCJI”), włączona flaga; environment production. Do 24.09.2026 (#595) — samo scalenie PR-a | Zastosowanie IaC; zmiany destrukcyjne tylko po zaznaczeniu wejścia `zmiany_destrukcyjne` (domyślnie zablokowane) | Brak zarządzanego zastosowania infrastruktury. Nie zweryfikowano tu działania zewnętrznej akcji ani zgodności planu z apply na żywo. |
