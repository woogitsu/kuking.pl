...
| `pilny_alarm_nie_dotarl` | Adres jest ustawiony, a pilna sprawa nadal nie ma zleconego alarmu: worker zginął między zapisem sprawy a listem (`zalegly`) albo zlecenie listu rzuciło wyjątkiem (`nieudany`, wyjątek poszedł do `report()`). | Otwórz `/admin/sygnaly` i obejrzyj sprawę od razu. Komenda ponawia co godzinę; jeśli kod trzyma się dłużej niż godzinę, a przebieg harmonogramu kończy się błędem (kod 1), zlecenie listu pada za każdym razem i trafia do zgłaszania błędów. Jeśli przebieg kończy się kodem 0, a wynik komendy i dziennik mówią o dobowym suficie alarmów (audyt B8-02, `KUKING_MODEL_ALARM_SUFIT`), sprawy czekają na odnowienie sufitu następnej doby — sonda widzi je przez `alarm_sonda_godzin` (72 h), panel zawsze. Sprawdź pocztę (`checks.poczta`, `checks.listy`) i log. Ręcznie: `php artisan kuking:doslij-pilne-alarmy`. |
