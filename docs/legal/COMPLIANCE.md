...
| P0 | Ciasteczko sesji na produkcji leci wyłącznie po HTTPS | `CiasteczkoSesjiJestSecureNaProdukcjiTest` — na `APP_ENV=production` brak zmiennej daje `secure=true`, więc **przez pominięcie** nie da się dostać ciasteczka bez tej flagi. Stan faktyczny potwierdzony niezależnie pomiarem: `https://kuking.pl/login` oddaje `Set-Cookie: kuking-session=…; secure; httponly; samesite=lax` [pomiar cudzy: sesja prowadząca floty, 20.09.2026] | Nie |
