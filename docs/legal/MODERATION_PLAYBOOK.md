...
> To dokument roboczy dla zespołu moderacji (1–2 osoby). Nie jest to porada prawna — w sprawach niejasnych (groźby karalne, podejrzenie CSAM, żądania organów ścigania) zawsze konsultuj się z prawnikiem lub od razu zgłaszaj do organów, zgodnie z procedurą zero-tolerancji niżej. Powiązane dokumenty: `COMPLIANCE.md` (podstawy prawne, obowiązki DSA), `REGULAMIN_DRAFT.md`.

---

## 1. Zasady Kuking (wersja dla użytkowników)

Krótka, ludzka wersja — do publikacji na `/zasady` i linkowania z formularza zgłoszenia. Pisana tak, żeby 65-latek zrozumiał ją bez czytania dwa razy.
...
| Podszywanie się | Konto udające inną, realną osobę (np. znanego kucharza) | Zawieszenie konta do wyjaśnienia | Jeśli potwierdzone → blokada trwała | Ukryć profil do czasu wyjaśnienia | Tak, z prośbą o wyjaśnienie/dowód tożsamości |
...
| Niebezpieczna porada zdrowotna/żywieniowa | "Soda oczyszcza z raka", niebezpieczne przetwory bez zasad bezpieczeństwa | Ukrycie + neutralny komentarz/ostrzeżenie edukacyjne, nie oskarżanie | Przy uporczywym powtarzaniu → blokada czasowa | Ukryć, dodać kontekst jeśli to możliwe technicznie | Tak, rzeczowo, bez oceniania |
| Nieletni na koncie | Wpis/profil sugerujący wiek poniżej 16 lat | Zawieszenie konta do wyjaśnienia | Potwierdzone → trwałe zamknięcie konta z informacją | Ukryć profil do wyjaśnienia | Tak, z wyjaśnieniem zasad wieku |
...
| CSAM / seksualizacja dzieci | Jakakolwiek treść tego typu | **Zero tolerancji — patrz sekcja 6** | Natychmiastowe zgłoszenie do organów | Usunąć + zabezpieczyć dowód zgodnie z procedurą | Nie kontaktować się z autorem — patrz sekcja 6 |
| Groźby / zagrożenie życia | Wypowiedź wskazująca na realne zagrożenie życia (własnego lub cudzego) | **Zgłoszenie do organów — patrz sekcja 6** | — | Ukryć treść, zachować kopię | Ostrożnie, priorytet to bezpieczeństwo, nie moderacja |
...
| **Moderator** | Przeglądać kolejkę zgłoszeń, ukrywać/usuwać treść, wysyłać ostrzeżenia, nakładać blokady czasowe (do ustalonego limitu, np. 30 dni), odrzucać zgłoszenia z uzasadnieniem |
| **Admin** | Wszystko co moderator + blokady trwałe, decyzje o odwołaniach, dostęp do audit logu, zarządzanie kontami moderatorów, decyzje w sprawach P0 |
...
### 4.2 Treść ukryta (widoczna tylko dla autora)
...
>
> Zasady Kuking znajdziesz tutaj: [link]. Jeśli masz pytania, po prostu odpisz.
>
> Pozdrawiamy,
> Zespół Kuking
...
>
> Jeśli masz dodatkowe informacje, których nie uwzględniliśmy, odpisz na tego maila.
>
...
- Nowe konto z linkiem w bio do domeny niezwiązanej z gotowaniem (sklep, kurs, "zarabianie") → priorytet w kolejce triage.
- Perceptual hash (`media.perceptual_hash` — już w schemacie) wykorzystany do wykrywania masowego wgrywania tego samego zdjęcia przez różne konta w krótkim czasie → sygnał farmy kont.

### Rate limity rekomendowane (patrz też `SECURITY_BASELINE.md` dla pełnej listy technicznej)

- Rejestracja: max 3 konta / IP / 24h.
- Komentarz: max 1 na 10 sekund, max 20/godzinę dla nowego konta (<7 dni).
- Zgłoszenie treści: max 20/dzień na użytkownika (żeby jedna osoba nie zalała kolejki).
...
| **Cudze zdjęcie podpisane jako własne** | Usunięcie + wiadomość do autora (szablon 4.1) + jeśli zgłaszający jest właścicielem oryginału, poinformowanie go, że treść usunięto. |
...
1. **Nie rozpowszechniaj, nie kopiuj, nie przesyłaj dalej** podejrzanej treści (nawet wewnętrznie, np. e-mailem między moderatorami) — to samo w sobie może być czynem karalnym. Ogranicz dostęp do minimum (np. status treści w bazie na `removed`/`quarantine`, bez ściągania pliku na dysk lokalny).
...
- Powiązane: `/tmp/claude-0/-home-user-kuking-pl/.../scratchpad/agents/legal/COMPLIANCE.md` (podstawy prawne DSA/RODO cytowane w tym dokumencie)
- Wewnętrzne źródło produktowe: `bp/kuking-platform/docs/MODERATION.md` (założenia produktowe, na których oparto ten podręcznik)
