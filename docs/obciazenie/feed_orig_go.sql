select "posts".*,
  (select count(*) from "comments"
     where "comments"."post_id" = "posts"."id"
       and not exists (select 1 from "blocks"
             where ("blocks"."blocker_id" = $1 and "blocks"."blocked_id" = "comments"."author_id")
                or ("blocks"."blocker_id" = "comments"."author_id" and "blocks"."blocked_id" = $1))
  ...
  (select count(distinct "zu"."id") from "users" as "zu"
     ...
     where "collection_items"."post_id" = "posts"."id"
       ...
       and not exists (select 1 from "blocks"
             where ("blocks"."blocker_id" = $1 and "blocks"."blocked_id" = "zu"."id")
                or ("blocks"."blocker_id" = "zu"."id" and "blocks"."blocked_id" = $1))) as "zapisow_count",
