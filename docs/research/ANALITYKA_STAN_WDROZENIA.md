...
| `App\Domain\Analytics\DrugiWpisW7Dni` | „Drugi wpis w 7 dni”: z osób, których pierwszy opublikowany wpis ma 7–90 dni, ile opublikowało drugi najpóźniej 7×24 h po pierwszym. Definicja w `docs/research/ANALITYKA.md` §1.5 (issue #29). Z wykluczeniami `CookEligibility`, same liczniki, procent dopiero od 10 osób. |
...
| `App\Domain\Analytics\ZrobiePonownie` | „Zrobię ponownie” w trzech stanach (tak / nie / brak odpowiedzi), osobno dla cudzych i własnych przepisów, ostatnie 30 dni — definicja mianownika w `docs/research/ANALITYKA.md` §1.2 (issue #1509). Z wykluczeniami `CookEligibility`. |
| `App\Domain\Analytics\ZapisDoUgotowania` | `save → cooked w 30 dni`: pary osoba–przepis z pełnej kohorty pierwszych zapisów sprzed 30–60 dni, konwersja = ugotowanie po zapisie i w ciągu 30 dni; zapisy młodsze niż 30 dni osobno — definicja w `docs/research/ANALITYKA.md` §1.2 (issue #1015). Z wykluczeniami `CookEligibility`. |
| `App\Domain\Analytics\HistoriePrzepisow` | „Historie przepisów”: jaka część opublikowanych przepisów (ostatnie 90 dni, wszystkie widoczności) ma wypełnione „od kogo”, historię, rok rodzinny, gotowy skan i `source_type = family` — plus zbiorcze „choć jeden ślad” i „rodzinny ze śladem”. Definicje w `docs/research/ANALITYKA.md` §1.2 (issue #1045). Z wykluczeniami `CookEligibility`, bez treści pól. |
