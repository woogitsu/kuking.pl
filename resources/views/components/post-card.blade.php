        ...
        @auth
            ...
            @if($zapisy->czyZapisany($post))
                @if($zeszyt !== null && auth()->id() === $zeszyt->owner_id)
                    {{--
                        WYJĄTEK OD „ODNOŚNIK, NIE PRZYCISK KASUJĄCY" WYŻEJ —
                        I TO CELOWO (issue #776).

                        Zdanie „wyjąć z zeszytu można w samym zeszyku" stało
                        w komentarzu obok od miesiąca, ale nie odpowiadał mu
                        żaden przycisk: `collections.unsave-post` istniał,
                        w tym widoku nie było jak go dotknąć bez ręcznego
                        DELETE. Tu, w środku KONKRETNEGO zeszytu, nie ma już
                        ryzyka „podwójne kliknięcie „Zapisuję” w feedzie samo
                        się cofnie" — to inny ekran, w innym celu otwarty,
                        więc przycisk usuwający jest bezpieczny.

                        `collection_id` WSKAZUJE TEN JEDEN ZESZYT (issue #775):
                        usuwa lokalnie, nie ze wszystkich własnych zeszytów.
                    --}}
