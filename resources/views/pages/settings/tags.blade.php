{{--
    ...
    * każde naciśnięcie zabiera ze sobą CAŁY wybór, bo wysyła formularz —
    ...
    PRZYCISKI PRZEGLĄDANIA MAJĄ WŁASNY `formaction` (decyzja właściciela
    z 20.09.2026, punkt 1). Cały formularz nadal wysyła się jednym POST-em
    bez JavaScriptu — `formaction` to zwykły atrybut HTML5, wspierany od
    dawna, nie skrypt, który mógłby się nie dociągnąć. Dzięki niemu filtr
    i „Pokaż kolejne…" trafiają na `settings.tags.przegladaj`, z WŁASNYM
    koszykiem limitera (300/10 min), a „Zapisz" zostaje na
    `settings.tags.update`, pod dawnym, ostrożnym 30/10 — przeglądanie
    tagu przestało zjadać budżet zapisu i odwrotnie. Kolejność w dokumencie
    (patrz wyżej) i tak decyduje o Enterze — `formaction` na nią nie wpływa.
--}}
        ...
        <form method="POST" action="{{ route('settings.tags.update') }}">
            ...
            <div class="field tagi-filtr">
                ...
                <div class="tagi-filtr-wiersz">
                    ...
                    <button class="btn btn-secondary" type="submit" name="akcja" value="filtruj"
                            formaction="{{ route('settings.tags.przegladaj') }}">Pokaż pasujące</button>
                    ...
                    @if($szukaj !== '')
                        <button class="btn btn-secondary" type="submit" name="akcja" value="wszystkie"
                                formaction="{{ route('settings.tags.przegladaj') }}">Pokaż wszystkie tagi</button>
                    @endif
                </div>
            </div>
            ...
            @if($pozostalo > 0)
                <p class="tagi-doladuj">
                    <button class="btn btn-secondary" type="submit" name="akcja" value="wiecej"
                            formaction="{{ route('settings.tags.przegladaj') }}">
