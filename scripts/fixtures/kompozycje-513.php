...
echo json_encode(['sciezki513' => ['/witaj/zainteresowania', '/powiadomienia'], 'tagi513' => TagPromotion::count(), 'bezCelu513' => $owner->notifications()->where('data->pomiar', '513')->whereNull('actor_id')->firstOrFail()->id, 'akcje513' => $owner->notifications()->where('data->pomiar', '513')->whereIn('type', [Notification::TYPE_COOKED, Notification::TYPE_COMMENT, Notification::TYPE_REPLY, Notification::TYPE_FOLLOW, Notification::TYPE_SAVED])->get()->filter(fn ($n) => $n->actor_id !== null && $n->read_at === null)->map(fn ($n) => ['id' => $n->id, 'url' => $n->adresDocelowy()])->values()], JSON_THROW_ON_ERROR);
