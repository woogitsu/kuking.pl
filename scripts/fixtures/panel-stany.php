...
try {
    ...
    DB::transaction(function () use ($host, $gate, $author, $marker, $database, $handle): void {
        ...
        // Zgłaszający spoza obsługi (#1408, D-244): `$host` loguje się w teście
        // przeglądarkowym i rozstrzyga te sprawy, a własnego zgłoszenia nie wolno.
        $resolved = Report::create(['reporter_id' => $author->id, 'target_type' => 'post', 'target_id' => $hidden->id, 'source' => Report::SOURCE_COMMUNITY, 'reason' => 'other', 'details' => 'Dane poglądowe: sprawa rozpatrzona.', 'status' => Report::STATUS_RESOLVED, 'resolved_by' => $gate->id, 'resolved_at' => now()]);
        $open = Report::create(['reporter_id' => $author->id, 'target_type' => 'post', 'target_id' => $openPost->id, 'source' => Report::SOURCE_COMMUNITY, 'reason' => 'other', 'details' => 'Drugi lokalny formularz do odbioru walidacji.', 'status' => Report::STATUS_OPEN]);
