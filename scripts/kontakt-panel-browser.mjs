...
try {
  ...
  await page.locator('textarea[name=odpowiedz]').fill('Szkic odpowiedzi 845.');
  await page.locator('[name=status][value=in_progress]').check();
  await page.getByRole('button', { name: 'Zapisz', exact: true }).click();
  await page.waitForLoadState();
  assert.equal(await page.locator('textarea[name=odpowiedz]').inputValue(), 'Szkic odpowiedzi 845.');
  assert.equal(await page.locator('article.wizard-row').count(), 0);
  step = 'notatka-przy-wysylce';
  const firstKey = await page.locator('#odpowiedz-formularz input[name=reply_key]').inputValue();
  await page.locator('textarea[name=handler_note]').fill('Szkic notatki 845.');
  await page.getByRole('button', { name: 'Wyślij odpowiedź', exact: true }).click();
  await page.waitForLoadState();
  assert.equal(await page.locator('textarea[name=handler_note]').inputValue(), 'Szkic notatki 845.');
  assert.equal(await page.locator('textarea[name=odpowiedz]').inputValue(), '');
  ...
  await page.locator('#odpowiedz-formularz input[name=reply_key]').evaluate((el, key) => { el.value = key; }, firstKey);
  await page.locator('textarea[name=odpowiedz]').fill('Drugi świadomy list.');
  await page.getByRole('button', { name: 'Wyślij odpowiedź', exact: true }).click();
  await page.waitForLoadState();
  assert.equal(await page.locator('article.wizard-row').count(), 1);
  ...
  await page.locator('textarea[name=handler_note]').fill('Nowsza notatka 846.');
  await page.locator('[name=status][value=done]').check();
  await page.getByRole('button', { name: 'Zapisz', exact: true }).click();
  await page.waitForLoadState();
  await stale.locator('textarea[name=handler_note]').fill('Starsza notatka 846.');
  ...
  assert.equal(await stale.locator('textarea[name=handler_note]').inputValue(), 'Starsza notatka 846.');
  ...
  await stale.locator('textarea[name=odpowiedz]').fill('Szkic klawiaturą.');
  ...
  assert.equal(await stale.locator('textarea[name=odpowiedz]').inputValue(), 'Szkic klawiaturą.');
  assert.ok(await stale.locator('textarea[name=odpowiedz]').evaluate(el => parseFloat(getComputedStyle(el).fontSize) >= 18));
  ...
  await zoomPage.locator('textarea[name=odpowiedz]').fill('Szkic przy zoomie 200%.');
  await zoomPage.getByRole('button', { name: 'Zapisz', exact: true }).click(); await zoomPage.waitForLoadState();
  assert.equal(await zoomPage.locator('textarea[name=odpowiedz]').inputValue(), 'Szkic przy zoomie 200%.');
