...
export async function sprawdzNegatywNawigacji({ browser, adres, sesja, outputDir }) {
  if (process.env.GITHUB_ACTIONS === 'true' || process.env.DB_PORT !== '55439'
    || process.env.DB_DATABASE !== 'kuking_638_negative' || !['127.0.0.1', 'localhost'].includes(new URL(adres).hostname)) throw new Error('NAV638_IZOLACJA');
  ...
  const probe = async () => {
    ...
    try {
      ...
      await page.evaluate(async () => { document.documentElement.dataset.textScale='100'; await document.fonts.ready; await new Promise(ok=>requestAnimationFrame(()=>requestAnimationFrame(ok))); });
      return await sprawdzEtykietyDolnejNawigacji(page);
    } finally { await context.close(); }
  };
  const baseline = await probe();
  const original = readFileSync(source, 'utf8');
  const block = /\[data-marka\] :where\(\.bottom-nav:not\(\.bottom-nav-panel\)\) > \.bottom-nav-item \{[^}]+\}/g;
  ...
  writeFileSync(join(outputDir,'nav638-negative.json'),JSON.stringify({backup,source:'resources/css/marka-rama.css',before,restoredMd5:md5(),restoredMtimeNs:statSync(source,{bigint:true}).mtimeNs.toString(),detected,failure:failure?.startsWith('NAV638_')?failure.split(' ')[0]:'NIEZNANY',baseline,positive},null,2));
