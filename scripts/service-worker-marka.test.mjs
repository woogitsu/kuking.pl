...
const caches = {
  async open(name) {
    ...
    return {
      ...
      async put(r, value) { if (cacheFull) throw new Error('QuotaExceededError'); store.set(key(r), value); },
    };
  },
...
async function get(path, mode = 'cors', method = 'GET') {
  let work;
  events.fetch({ request: { url: `https://kuking.test${path}`, mode, method }, respondWith(p) { work = p; } });
  return work;
}
