...
class KompozycjaStartuTest extends TestCase
{
    ...
    public function test_home_i_root_maja_te_same_sekcje_i_dzialajace_adresy(): void
    {
        ...
        foreach (['/home', '/'] as $path) {
            ...
            $this->assertSame(route('about'), $dom->query($header.'//a')->item(0)?->getAttribute('href'));
            $composer = $this->classQuery('composer');
            $this->assertSame('Co dziś gotujesz? Dodaj zdjęcie', $dom->query($composer)->item(0)?->getAttribute('aria-label'));
            $this->assertSame(route('posts.create'), $dom->query($composer)->item(0)?->getAttribute('href'));
            ...
            $this->assertSame(route('discover'), $dom->query($intro.'//a')->item(0)?->getAttribute('href'));
