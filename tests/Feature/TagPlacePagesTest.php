...
class TagPlacePagesTest extends TestCase
{
    ...
    public function test_only_promoted_cards_have_collages_and_editorial_notes_are_escaped(): void
    {
        $tag = Tag::factory()->create();
        $this->photoPost($tag);
        $plain = Tag::factory()->create();
        $this->photoPost($plain);
        ...
        $this->assertSame(0, $xpath->query('//nav[@aria-label="Wszystkie tagi, alfabetycznie"]//img')->length);
