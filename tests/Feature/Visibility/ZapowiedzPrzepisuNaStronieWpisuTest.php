 ...
 *
 * BRAMKA JUŻ STOI W MAIN — TEN PLIK JEJ NIE DOKŁADA (2026-09-22)
 * Ten test powstał razem z własną poprawką w `PostPolicy::view()`. Zanim
 * zdążył wejść, main zamknął tę samą dziurę w tym samym miejscu:
 * commit `c9440094`, `app/Policies/PostPolicy.php` — warunek
 * `czyJestZapowiedziaPrzepisu()` + `RecipePolicy::view()` tuż przed
 * `match ($post->visibility)`. Druga bramka obok tamtej byłaby martwym
 * kodem udającym zabezpieczenie, więc tutaj NIE MA ŻADNEJ ZMIANY W KODZIE
 * PRODUKCYJNYM — zostaje sam pomiar.
 *
 * ZMIERZONE: z bramką main wszystkie 11 testów jest zielonych; po
 * wycięciu tamtego warunku z `PostPolicy::view()` padają dokładnie te
 * pięć, które pytają o wyciek, a sześć kontroli dodatnich zostaje
 * zielonych. Czyli plik mierzy bramkę main, a nie sam siebie.
 *
 * CO WNOSI PONAD `StronaWpisuBramkaPrzepisuTest` z main
 * Tamten plik sprawdza przepis `followers` i publiczny. Tutaj dochodzą
 * dwie osobne przyczyny ukrycia przepisu, których tam nie ma:
 * `private` oraz zdjęcie przez MODERACJĘ (status) — obie padły w próbie
 * z wyciętą bramką, więc to realne pokrycie, a nie powtórka.
 */
