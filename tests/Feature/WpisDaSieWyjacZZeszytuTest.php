...
class WpisDaSieWyjacZZeszytuTest extends TestCase
{
     ...
     *
     * NAZWA ZALEŻY OD EKRANU, BO ZALEŻY OD ZAKRESU (D-226). W środku
    ...
    public function test_na_ekranie_ze_stanem_zapisu_stoi_przycisk_wyjecia(string $ekran, string $nazwaPrzycisku): void
    {
        ...
        // w zeszycie". W samym zeszycie tego zdania CELOWO nie ma (D-226):
        // prowadzi do listy zeszytów, a człowiek stojący W zeszycie już wie,
        // że wpis tam leży.
        if ($ekran === 'zeszyt') {
            $this->assertStringNotContainsString('Masz to w zeszycie', $html, 'W zeszycie odnośnik stanu nie ma już stać (D-226).');
        } else {
    ...
    /**
     * DOKŁADNIE JEDNA DROGA WYJĘCIA NA EKRAN — TO JEST SEDNO D-226.
     *
    ...
    public function test_na_ekranie_jest_dokladnie_jedna_droga_wyjecia(string $ekran, string $nazwaPrzycisku): void
    {
        ...
        $this->assertCount(
            ...
            $formularze,
            sprintf(
                'Ekran „%s” pokazuje %d dróg wyjęcia wpisu z zeszytu zamiast jednej. Dwie prawie identyczne nazwy o różnym zasięgu są dla tej grupy gorsze niż brak którejkolwiek (D-226).',
                $ekran,
                count($formularze),
            ),
        );
    ...
    /**
     * WSZYSTKIE formularze wyjęcia tego wpisu — bo sednem D-226 jest ICH
