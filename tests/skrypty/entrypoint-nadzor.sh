...
for cel in kolejka www harmonogram; do
...
jeden_przebieg_kolejki() { if [[ "$CEL" == kolejka ]]; then return 23; else trwaj; fi; }
harmonogram_raz() { if [[ "$CEL" == harmonogram ]]; then return 24; else trwaj; fi; }
frankenphp() { if [[ "$CEL" == www ]]; then return 25; else trwaj; fi; }
PROBA
)
CEL=$cel
case \"\${ROLE}\" in
